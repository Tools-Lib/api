...
router.post("/", (req, res) => {
  ...
  db.query(`SELECT UID, username, password FROM users WHERE username=?`, [username], (err, rows) => {
    if(err) throw err;
    if(!rows || !rows[0] || rows.length < 1) {
      ...
      bcrypt.compare(password, rows[0].password, function(err, result) {
        if(result) {
          ...
          db.query(`UPDATE users SET last_login='${new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds() + " " +new Date().getMonth() + "/" + new Date().getDate() + "/" + new Date().getFullYear()}' WHERE UID=${rows[0].UID}`);
